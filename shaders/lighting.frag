...
// Task 3: Spotlight (camera headlight, pos=origin in view space)
struct SpotLight {
    vec3  direction;   // view space, typically (0,0,-1)
    vec3  ambient;
    vec3  diffuse;
    vec3  specular;
...
uniform int       spotLightOn;  // 0 = off, 1 = on
...
vec3 calcSpotLight(SpotLight light, vec3 N, vec3 fragPos, vec3 V, vec3 texColor) {
    // Camera is at view-space origin; L points from fragment toward origin
    ...
    // Ambient is always applied; diffuse/specular are modulated by spot intensity
