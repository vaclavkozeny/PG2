...
// Change the constructor to call the correct method name

ShaderProgram::ShaderProgram(const std::filesystem::path & VS_file, const std::filesystem::path & FS_file) :
    ShaderProgram{textFileRead(VS_file), textFileRead(FS_file)} {} // Changed read_text_file to textFileRead
...
GLint ShaderProgram::getAttribLocation(const std::string & name) {
    // We must call .c_str() to convert std::string to const GLchar*
    GLint loc = glGetAttribLocation(ID, name.c_str()); 
    
    if (loc == -1) {
        std::cerr << "No vertex attribute with name: " << name 
                  << ", or reserved name (starting with gl_)\n";
...
ShaderProgram::~ShaderProgram() {
    if (ID != 0) {
        if (currently_used_ID == ID) deactivate();
        glDeleteProgram(ID);
    }
}
